        ...
        if len(sys.argv) == 2:
            ...
            if len(sys.argv) > 3:
                if sys.argv[3] == "gen_keys":
                    a = Ntru(Integer(sys.argv[4]), Integer(sys.argv[5]), Integer(sys.argv[6]), Integer(sys.argv[7]))
                    a.gen_keys(sys.argv[8], sys.argv[9])
                    print("Criptosistema NTRU instanciat amb els paràmetres N={}, p={}, q={}, d={}".format(sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7]))
